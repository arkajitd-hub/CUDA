...
void jacobi_gpu_teams(double* T, double* T_new, int max_iterations, int size) {
    int iteration = 0;
    ...
    while (residual > MAX_RESIDUAL && iteration < max_iterations) {
        ...
        #pragma omp target teams distribute map(to: T[0:size*size]) map(from: T_new[0:size*size])
        ...
        #pragma omp target teams distribute reduction(max: residual) map(tofrom: T[0:size*size]) map(to: T_new[0:size*size])
        for (int i = 1; i < size - 1; i++) {
            for (int j = 1; j < size - 1; j++) {
...
int main(int argc, char* argv[]) {
    ...
    jacobi_gpu_teams(T, T_new, max_iterations, size);
    auto end = chrono::high_resolution_clock::now();

    chrono::duration<double> elapsed = end - start;
    cout << "Time taken for GPU Teams: " << elapsed.count() << " s, N=" << N
